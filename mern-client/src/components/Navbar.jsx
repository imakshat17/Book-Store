...
function Navbar() {
  ...
  return (
   <header className='w-full bg-transparent fixed top-0 left-0 right-0 transition-all ease-in duration-300'>
    <nav className={`py-4 lg:px-24 px-4 ${isSticky ? 'sticky top-0 left-0 right-0 bg-blue-300' : ''}`}style={{ background: '#8585ec' }}>
        <div className='flex justify-between items-center text-base gap-8'>
            ...
            <ul className='md:flex space-x-12 hidden'>
                {
                    navItems.map(({link,path})=><Link key={path} to={path}className='block text-base text-black uppercase cursor-pointer hover:text-blue-700 bg-blue'>{link}</Link>)
                }
            </ul>
            {/* btn for lg devices*/}
